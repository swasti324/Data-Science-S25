...
assertthat::assert_that(
  tibble(x = 1, y = 0) %>% # Pick a point *inside* the circle
...
assertthat::assert_that(
  tibble(x = 0.71, y = 0.71) %>% # Pick a point *outside* the circle
    mutate(stat = stat(x, y)) %>%
    pull(stat) %>%
    .[[1]] == 0,
  # ???, # Write the correct value of stat() here
...
- You chose a correct value of `stat(x, y)` when `x, y` is *outside* the
  ...
  - I chose (0.71, 0.71) as that is just right out of the bounds of the
    circle. I wanted to ensure that the function passed this edge case.
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the
  circle. Why did you choose this value?
  - I chose (1, 0) as I saw that as another edge case as it is right
    inside the bounds of the circle.
...
df_q3 <-
  df_q1 %>%
  mutate(stat = stat(x, y)) %>%
  summarise(pi_est = mean(stat))
# collect all the points in the circle where stat == 4
# divide by the total number of points to equal pi
...
![](c07-monte-carlo-assignment_files/figure-gfm/unnamed-chunk-7-1.png)<!-- -->

*Observations*

- What is a range of plausible values, based on the sampling
  distribution you’ve generated?
  - The full range is from around 3.115 to 3.165
  - the data displays a normal distribution centered around 3.14,
    indicating a reasonable result on average.
...
df_q5 <-
  ...
  summarize(
    ...
    pi_mean = mean(pi_est),
    se = sd(pi_est) / sqrt(n),
    pi_lo = pi_mean - 1.96 * se,
    pi_up = pi_mean + 1.96 * se
  )
...
```
    ...
    ## # A tibble: 1 × 4
    ##   pi_mean        se pi_lo pi_up
    ##     <dbl>     <dbl> <dbl> <dbl>
    ## 1    3.14 0.0000332  3.14  3.14
...
df_q1 %>%
  ...
  summarise(
    pi_est = mean(stat),
    se = sd(stat) / sqrt(n),
    pi_lo = pi_est - qnorm(1 - (1 - 0.99) / 2) * se,
    pi_hi = pi_est + qnorm(1 - (1 - 0.99) / 2) * se
  )
```

    ## # A tibble: 1 × 4
    ##   pi_est      se pi_lo pi_hi
    ##    <dbl>   <dbl> <dbl> <dbl>
    ## 1   3.14 0.00735  3.12  3.16
...
- How closely do your bootstrap CI and CLT CI agree?
  - The CLT has a larger standard error compared to the bootstrap.
    However they do center around a similar average
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - Yes, absolutely for a rough estimate.
  - Yes perhaps for a precise estimate, however ideally only if the
    sample size is incredibly high.
